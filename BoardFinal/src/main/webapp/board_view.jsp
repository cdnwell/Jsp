	...
	$(function(){
		...
		$(".btn_comment_hate").click(function(){
			...
			$.ajax({
				...
				success : function(r){
					if(r == "1")
						alert("이 댓글을 싫어요하셨습니다.");
					else
						alert("이 댓글을 싫어요를 취소하셨습니다.");
					location.reload();
...
<body>
	<table>
		...
		<tr>
			<td colspan="2">
				<a href="#" id="btn_like">좋아요 ${requestScope.board_dto.blike }</a>
				<a href="#" id="btn_hate">싫어요 ${requestScope.board_dto.bhate }</a>
			</td>
		</tr>
	...
	<form action="commentWrite.do" method="post" class="comment_box">
		<textarea name="comment" placeholder="댓글 내용을 입력하세요"></textarea>				
		<input type="hidden" name="bno" value="${requestScope.board_dto.bno }">
		<button>댓글작성</button>
	</form>
	<hr>
