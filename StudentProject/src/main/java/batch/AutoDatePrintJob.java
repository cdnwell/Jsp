...
//		System.out.println(sdf.format(date));
		...
		HashMap<StudentDTO,Integer> map = StudentService.getInstance().selectUnderAverage();  
		
		System.out.println("이번학기 학사경고 대상자");
		
		for(StudentDTO key : map.keySet()){
			int value = map.get(key);
			System.out.println(key.getSno()+" "+key.getName()+" "+value +" " 
					+ key.getScore());
		}
